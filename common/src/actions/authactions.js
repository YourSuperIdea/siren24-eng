...
export const fetchUser = () => (dispatch) => (firebase) => {
  const { auth, singleUserRef, settingsRef } = firebase;
    ...
    if (user) {
      try {
        fetch(`https://us-central1-seradd.${mainUrl}/baseset`, {
          ...
          body: JSON.stringify({
            license: PurchaseDetails.CodeCanyon_Purchase_Code
              ? PurchaseDetails.CodeCanyon_Purchase_Code
              : " ",
            contact_email: PurchaseDetails.Buyer_Email_Address
              ? PurchaseDetails.Buyer_Email_Address
              : " ",
          ...
          .then((res) => {
            if (res.success) {
              settingsRef.once("value", (settingdata) => {
                ...
                if (
                  (password_provider_found && settings.email_verify) ||
                      ...
                      if (snapshot.val()) {
                        ...
                        if (user.profile.approved) {
                          ...
                          dispatch({
                            ...
                            payload: {
                              code: language.auth_error,
                              message: language.require_approval,
                            },
                          });
                        }
                      }
                  ...
                  dispatch({
                    ...
                    payload: {
                      code: language.auth_error,
                      message: language.email_verify_message,
                    },
                  });
                }
              });
...
export const validateReferer = async (referralId) => {
  const response = await fetch(
    `${cloud_function_server_url}/validate_referrer`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const checkUserExists = async (regData) => {
  const response = await fetch(
    `${cloud_function_server_url}/check_user_exists`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const updateProfile =
  (userAuthData, updateData) => (dispatch) => (firebase) => {
